...
export const backfillUsers = onCall(async (req) => {
  ...
  if (req.auth.uid !== ADMIN_UID) throw new HttpsError("permission-denied", "Not allowed.");
  ...
  for (const doc of companiesSnap.docs) {
    ...
    if (!ownerUid) {
      skipped.push({ companyId, reason: "missing ownerUid" });
      continue;
    }
    ...
    batch.set(
      userRef,
      {
        ...
        createdAt: admin.firestore.FieldValue.serverTimestamp(), // merge keeps existing createdAt if already set
      },
      { merge: true }
    );
